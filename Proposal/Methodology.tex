...
Para extender el análisis del proceso $X$, como cadena de Markov, se introducen las siguientes definiciones.\\
...
\begin{definition}
	Sea $X =\{X_{t}\}_{t\in \N}$ una cadena de Markov con espacio de estados $S$. Se dice que $X$ es una cadena de Markov absorbente posee al menos un estado absorbente y si partiendo de cualquier estado $s\in S$ es posible llegar a algún estado absorbente, no necesariamente en un único paso.
\end{definition}

Aludiendo a las definiciones previas, se tiene que la cadena de Markov $X$ propuesta para modelar el proceso de siembra es una cadena de Markov absorbente. De esta manera, es posible preguntarse por cuál es el número de días que se espera transcurran hasta que el proceso de siembra fracase si dio inicio el $i$-ésimo día del año, con $i\in\{1,2,\ldots,365\}$.\\
...
\begin{definition}[Forma canónica]
	Sea $X$ una cadena de Markov a tiempo discreto con un espacio de estados $S$ de cardinalidad $s<\infty$ y con matriz de transición $A\in\mathcal{M}_{s\times s}(\R)$. Además, sean $r, t\in \N$ tales que $r+t=s$, donde $r$ denota la cantidad de estados absorbentes, mientras que $t$ denota el número de estados transitorios. Se dice que $A$ está en forma canónica si admite la siguiente representación \begin{equation}
		A= \begin{bNiceMatrix}[%
			first-row, 
			first-col]
		...
		\end{bNiceMatrix},
	\end{equation}donde $\mathbf{Q}\in \mathcal{M}_{t\times t}(\R)$, $\mathbf{R}\in \mathcal{M}_{t\times r}(\R)$, $\mathbf{0}\in \mathcal{M}_{r\times t}(\R)$ e $\mathbf{I}\in \mathcal{M}_{r\times r}(\R)$.
\end{definition}


Con la definición anterior, se introduce el concepto de matriz fundamental que se presenta a continuación.\\

\begin{definition}[Matriz fundamental]
	Sea $X$ una cadena de Markov discreta a tiempo discreto tal que matriz de transición esté en forma canónica \begin{equation*}
		A= \begin{bNiceMatrix}[%
			first-row, 
			first-col]
			& \mathrm{Transitorios} & \mathrm{Absorbentes}  \\
			...
			\mathrm{Absorbentes} & \mathbf{0} & \mathbf{I}  \\
		\end{bNiceMatrix},
	\end{equation*}entonces la matriz $\mathbf{N}\in \mathcal{M}_{t\times t}(\R)$ dada por \begin{equation}
	\mathbf{N}=(I_t- Q)^{-1},
	\end{equation}donde $I_t\in \mathcal{M}_{t\times t}(R)$ denota la matriz identidad, se denomina la matriz fundamental de $X$.
\end{definition}
...
\begin{theorem}[Tiempo esperado para la absorción]
	Sea $X$ una cadena de Markov absorbente con espacio de estados $S$, donde $t\in \N$ son transitorios, y con $\mathbf{N}$ su matriz fundamental. Si $\mu=(\mu_1, \mu_2,\ldots, \mu_t)^T$ es el vector cuya $i$-ésima entrada,  $\mu_i$, denota  el número de pasos esperado antes de que $X$ sea absorbida, dado que comenzó en el estado $s_i\in S$ entonces \begin{equation}
	\mu = \mathbf{N} \mathbf{1},
	\end{equation}donde $\mathbf{1}\in\R^{t}$ es tal que $\mathbf{1}=(1,1,\ldots,1)^T$.\label{mu} 
\end{theorem} 

El resultado anterior puede robustecerse por aludir al libro de Kemeny \textit{et al.} (1976) \cite[Capitulo 3]{Kemeny_1976} sobre la varianza del número de pasos para que una cadena de Markov absorbente sea absorbida, habiendo partido de un estado transitorio arbitrario, que se presenta a continuación.\\

\begin{theorem}[Varianza del tiempo para la absorción]
	Sea $X$ una cadena de Markov absorbente con espacio de estados $S$, donde $t\in \N$ son transitorios, y con $\mathbf{N}$ su matriz fundamental. Si $\sigma^2=(\sigma^2_1, \sigma^2_2,\ldots, \sigma^2_t)^T$ es el vector cuya $i$-ésima entrada,  $sigma^2_i$, denota la varianza del número de pasos antes de que $X$ sea absorbida, dado que comenzó en el estado $s_i\in S$ entonces  \begin{equation}
		\sigma^2 = (2\mathbf{N} - I_t)\mu -  \mu_{sq},
	\end{equation}donde $\mu=(\mu_1,\mu_2,\ldots,\mu_t)^T$ el vector del número esperado de pasos antes de la absorción y $\mu_{sq}\in \R^{t}$ se define por $\mu=(\mu^2_1,\mu^2_2,\ldots,\mu^2_t)^T$.
	\label{sigma_sq}
\end{theorem}

Con los resultados y definiciones previas, gráficamente, la matriz de transición mostrada en la Ecuación \eqref{TransitionProbs} se segmenta en los siguientes bloques, a fin de obtener su forma canónica. 
...
Al emplear el Teorema \ref{mu} y el teorema \ref{sigma_sq} de manera conjunta, se obtiene un criterio adicional. Para cada $i\in \{1,2,\ldots,365\}$, se propone al intervalo del (1-$\alpha$)100\% de confianza para el número de días para que la cadena $X$ se absorbida por $F$, dado que comenzó en el estado $D_i$, al intervalo \begin{equation}
	(\mu_{i} - z_{1-\alpha/2} \sigma_{i},\, \mu_{i}+ z_{1-\alpha/2}\sigma_{i}),
\end{equation}donde $\sigma_{i}=\sqrt{\sigma^2_{i}}$ y $z_{1-\alpha/2}$ denota al $(1-\alpha/2)$-ésimo cuantil de una distribución normal estándar. Asumiendo, por simplicidad, que el teorema del límite central es válido al tener registros agroclimáticos históricos de 64 años independientes y consecutivos.\\

De esta manera, se sugiere que la siembra se realice el $i$-ésimo día del año si \begin{equation}
	n^*< \mu_{i} - z_{1-\alpha/2} \sigma_{i}.
\end{equation}
