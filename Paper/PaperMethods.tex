...
\end{equation}para algún $\zeta^*\in [0,1]$. Es decir, bajo el esquema de trabajo anterior, se sugiere al $i$-ésimo día siempre que la emergencia de la semilla ocurra antes del fracaso del proceso. Con este criterio general, se plantean cuatro enfoques para evaluar la relación temporal entre ambos eventos y, en consecuencia, determinar la aptitud diaria para la siembra.\\



\subsection{Modelo: Criterio Markoviano de permanencia}
Las cadenas de Markov permiten modelar procesos cuya dinámica depende exclusivamente del estado inmediatamente anterior, sin considerar la trayectoria histórica previa del proceso. Al interpretar la Figura \ref{Transicion} como un diagrama de transición asociado a una cadena de Markov, la probabilidad indicada en la Ecuación \eqref{EmergencyProb} se calcula de la manera siguiente.\\

 \begin{equation}
	\begin{aligned}
		&P_{i}(\tau_F>n^*)=\prod_{t=0}^{n^*-1} \lambda_{(i+t) \bmod{365}}.
	\end{aligned}
	\label{product_lambdas}
\end{equation}


\subsection{Modelo: Criterio de disponibilidad de tiempo}
Al tener estados absorbentes, el análisis de las cadenas de Markov admite una alternativa a través de la teoría de cadenas de Markov absorbentes. Dado que, al partir del estado de fallo, la probabilidad de permanecer en tal estado es uno, la cadena de Markov propuesta es, en efecto, una cadena de Markov absorbente.\\

Empleando las herramientas discutidas por \citet[Capítulo 11]{Grinstead_2003} y \citet[Capítulo 3]{Kemeny_1976}  se obtiene que, al menos, el $\displaystyle\left(1-\frac{1}{\kappa^2}\right)100\%$ de las realizaciones que parten del estado $D_i$, $i\in{1,2,\ldots,365}$, presentan una duración comprendida en el intervalo \begin{equation*}
	(\mu_i- \kappa \sigma_i \,\mathbf{,}\, \mu_i+\kappa\sigma_i)
\end{equation*} antes de ser absorbidas por el estado $F$.\\

De esta forma, para cada $i\in{1,2,\ldots,365}$, se sugiere que la siembra se realice el $i$-ésimo día del año si
\begin{equation}
	n^* < \mu_i- \kappa \sigma_i,
\end{equation} donde $\mu_i$ denota la $i$-ésima entrada del vector $ \mathbf{N} \mathbf{1}$, $\sigma_i$ indica la raíz cuadrada de la $i$-ésima entrada del vector $(2\mathbf{N} - I_{365})\mu -  \mu_{sq}$ y $\kappa\in\R$ el parámetro de la desigualdad de Chebyshev.\\

Asimismo, $\mathbf{N}\in \mathcal{M}_{365\times 365}(\R)$ es la matriz fundamental asociada a la cadena de Markov,  $\mathbf{1}\in\R^{365}$ es el vector $\mathbf{1}=(1,1,\ldots,1)^T$, $I_{365}\in \mathcal{M}_{365\times 365}(\R)$ denota la matriz identidad y $\mu_{sq}\in \R^{365}$ se define por $\mu_{sq}=(\mu^2_1,\mu^2_2,\ldots,\mu^2_{365})^T$.\\

\subsection{Modelo: Criterio de alcanzabilidad fenológica eventual}
...
Aludiendo al trabajo de \citet[Capítulo 3]{Kemeny_1976} se tiene que al  partir del estado $D_i$, $i\in \{1,2,\ldots,365\}$, la probabilidad de visitar eventualmente el estado $D_{(i+n^*)\bmod 365}$ está dada por la entrada $h_{i,(i+n^*)\bmod 365}$ de la matriz $H \in \mathcal{M}_{365\time 365} (\R)$ definida por \begin{equation*}
	H=(\mathbf{N}-I_{365})(N_{dg})^{-1},
	\end{equation*}
donde $I_{365}\in \mathcal{M}_{365\times 365}(\R)$ denota la matriz identidad y $N_{dg}\in \mathcal{M}_{t\times t}(\R)$ es la matriz diagonal con la misma diagonal que la matriz fundamental $\mathbf{N}$.\\

De esta manera, para cada $i\in\{1,2,\ldots,365\}$, se sugiere realizar la siembra en el $i$-ésimo día del año si 
\begin{equation}
...
\subsection{Modelo: Criterio empírico de permanencia}

Al considerar la Figura \ref{Transicion} como un gráfica dirigida cuyos pesos son probabilidades, podemos emplear una metodología similar a la discutida en el trabajo de \cite{deAndaJauregui_2021}. Así, al permitir que caminatas aleatorias recorran la gráfica es posible estimar el valor de $P_{i}(\tau_F>n^*)$ a través de estudiar sus trayectorias y longitudes.\\

Con este enfoque, las caminatas aleatorias se inicializan en cada uno de los vértices. En particular, para cada $i\in \{1,2,\ldots,365\}$, se inicializa una caminata aleatoria que parte del vértice $D_i$ y cuya trayectoria llega, por lo menos, al vértice $D_{(i+n^*)\bmod 365}$, o bien, llega al vértice $F$ sin haber visitado el vértice $D_{(i+n^*)\bmod 365}$. Por repetir el proceso anterior $r^*\in \N$ veces para cada vértice, se propone a \begin{equation}
...
\begin{table}[h]
	\centering
	\begin{tabular}{c|p{5cm}|p{2.3cm}|p{5.8cm}}
		\hline
		\textbf{Parámetro} & \multicolumn{1}{c|}{\textbf{Descripción}} & \multicolumn{1}{c|}{\textbf{Valor}} & \multicolumn{1}{c}{\textbf{Fuente}} \\
		\hline
		$n^*$ & Número de días requeridos para que una semilla viable alcance la emergencia desde la siembra. & $$16.6\pm3.1$$ &  \cite{CHMIELEWSKI_2004}. \\
		\hline
		$\zeta^*$ & Umbral mínimo de probabilidad para considerar viable una fecha de siembra. & $$0.8$$ & \citet[p. 57]{FAO_2011}. \\
		\hline
		$r^*$ & Número de repeticiones de caminatas aleatorias para estimación empírica. & $$500$$ & \cite{deAndaJauregui_2021}. \\
		\hline
		$C^*$ & Centroide de la región de condiciones agroambientales óptimas para el desarrollo del cultivo. &$$(11.10,75,9,85)$$& Ver Tabla \ref{tab:optimal_conditions}
		\\
		\hline
		$C_{H,k}$ & Centroide de las condiciones agroambientales históricas observadas en el día $k$ del año. & 	N/A &  Ver Tabla \ref{table:Vars} \\
		\hline
		$d(\cdot,\cdot)$ & Métrica de distancia en el espacio de variables agroambientales. & Euclidiana \newline
		Manhattan & \begin{center}
			N/A
		\end{center}  \\
		\hline
		...
		\end{center}  \\
		\hline
		$\kappa$ & Parámetro en la desigualdad de Chebyshev. & $$4.47$$ &  \cite{daSilva_2019}.
		\\
		\hline
	\end{tabular}
